...
Jobs lens 显示当前 artifact 最近的 analysis jobs/pass 历史。它来自项目数据库中的 `analysis_jobs` 表，按 pass、status、profile、progress、objects、diagnostics 和时间展示，用来快速确认 `binary.parse`、`binary.triage`、`binary.cfg`、`binary.dataflow` 等带 Lab 前缀的 pass 结果。移动 Jobs 表格行时，右侧 Job Inspector 会显示选中 job 的 ID、run/artifact lineage、limits、timestamps、metadata、parameter snapshot、diagnostic snippets 和 log snippets。
...
- 回看最近一次启动 TUI 时加载到的历史状态。

当前 Jobs lens 是只读历史视图；live refresh、cancel/rerun 控制和真正异步调度仍是后续阶段。
...
1. 在 `Overview` 确认导入状态、对象计数和 Cockpit jobs 摘要。
2. 按 `J` 进入 `Analysis Jobs`，确认 `skipped` 是否只是 profile 降级。
3. 按 `g` 进入 `Triage Board`，看最高优先级 next action。
4. 按 `r` 进入 `Function Radar`，优先看高分函数。
5. 在函数上按 `Enter`，用 `Right` 进入 `Inspector`。
6. 在 Inspector 中用 `Up` / `Down` 选择 evidence，按 `Enter` 跳到字符串或 import。
7. 用 `:xrefs current` 看当前对象关系。
8. 用 `:tag`、`:note`、`:status` 留下分析记忆。
9. 用 `:finding new ...` 和 `:finding link ...` 形成结论。
10. 退出后用 `revdeck report` 导出报告。
